...
const StickySocialBar = () => {
  return (
    <motion.aside
      ...
      aria-label="Social media quick links"
    >
      {/* Gray Glassmorphism Background Container */}
      <div className="flex flex-col gap-2.5 p-2 rounded-2xl bg-neutral-400/50 dark:bg-neutral-900/60 backdrop-blur-md shadow-2xl shadow-black/20">
        {socialItems.map((item) => (
          <div key={item.label} className="relative group flex items-center">
            {/* Tooltip on hover (left side) with matching gray glass */}
            <span className="absolute right-full mr-3 whitespace-nowrap rounded-lg bg-neutral-900/80 backdrop-blur-md text-white text-xs px-2.5 py-1 font-medium shadow-md opacity-0 translate-x-2 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-200 pointer-events-none border border-neutral-700/50">
              {item.tooltip}
              <span className="absolute left-full top-1/2 -translate-y-1/2 border-4 border-transparent border-l-neutral-900/80" />
            </span>
            ...
            <Link
              ...
              rel={item.external ? "noopener noreferrer" : undefined}
              aria-label={item.label}
              className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all duration-300 transform group-hover:scale-105 shadow-md hover:shadow-lg ${item.bgColor}`}
            >
              <Image
                src={item.icon}
                alt={item.label}
